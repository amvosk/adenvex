...
class EnvelopeDetector(nn.Module):
    ...
    The class represents a PyTorch model that applies various filters, activation methods, 
    and downsampling techniques to input data. The purpose is to decode and interpret cortical 
    signals, emphasizing their envelope characteristics.
    ...
    dropout : float, optional (default=0)
        Dropout rate after the temporal convolution layer. It's useful when the `temporal_filter_size` is large.
    ...
    temporal_smoother_size : int, optional (default=3)
        Kernel size for the additional temporal convolution layer (only relevant if `use_temporal_smoother` is True).
    """
    def __init__(self, nchannels, nfeatures, temporal_filter_size=7, downsample_coef=1, dropout=0, 
                 activation_method='demodulation', downsample_method='avepool', fs_in=1000,
                 use_temporal_smoother=False, temporal_smoother_size=3):
        ...
        # Temporal filtering
        self.temporal_filter = nn.Conv1d(nfeatures, nfeatures, kernel_size=temporal_filter_size, 
                                         bias=False, groups=nfeatures, padding='same')
        ...
        # Temporal smoother
        self.temporal_smoother = (nn.Conv1d(nfeatures, nfeatures, kernel_size=temporal_smoother_size, 
                                            groups=nfeatures, padding='same') 
                                  if use_temporal_smoother else None)

        # Downsampler
        if downsample_coef > 1:
            if downsample_method == 'avepool':
                self.downsampler = nn.AvgPool1d(kernel_size=downsample_coef, stride=downsample_coef)
            elif downsample_method == 'resample':
                self.downsampler = torchaudio.transforms.Resample(fs_in, fs_in / downsample_coef)
    ...
    def spatial_patterns(self, x, nbatch=100):
        ...
        if (len(x.shape) == 2) or (len(x.shape) == 3 and x.shape[0] == 1):
            ...
            raise ValueError(f'Input with shape {x.shape} is not supported, len(input.shape) has to be in (2, 3).')
        
        spatial_patterns = torch.zeros((self.nfeatures, self.nchannels), dtype=torch.float32, device=self.spatial_filter.weight.device)
        for projection in range(self.nfeatures):
            temporal_filter = self.temporal_filter.weight.data[projection:projection+1]
            spatial_filter = self.spatial_filter.weight.data[projection]
            temporal_filter_ = einops.repeat(temporal_filter, '1 1 t -> c 1 t', c=self.nchannels)

            if not batched:
                x_filtered = F.conv1d(x, temporal_filter_, bias=None, padding='same', groups=self.nchannels)
                ...
                for batch in dataloader:
                    ...
                    batch_filtered = F.conv1d(batch, temporal_filter_, bias=None, padding='same', groups=self.nchannels)
                    batch_filtered = einops.rearrange(batch_filtered, 'c (b t) -> b c t', b=batch_size)
                    x_mean += torch.sum(batch_filtered, dim=(0,-1), keepdims=True) / batch.shape[-1]
                x_mean /= len(dataset)
                ...
                for batch in dataloader:
                    ...
                    batch_filtered = F.conv1d(batch, temporal_filter_, bias=None, padding='same', groups=self.nchannels)
                    batch_filtered = einops.rearrange(batch_filtered, 'c (b t) -> b c t', b=batch_size)
                    x_cov_batch = self._covariance(batch_filtered, unbiased=False, mean=x_mean)
        ...
        results = {
            'spatial_filters' : einops.rearrange(self.spatial_filter.weight.data, 'o i 1 -> o i'),
    ...
    def temporal_patterns(self, x, fs=1000, nyquist=500, nbatch=100):
        ...
        if not batched:
            ...
            frequencies_input, spectrum_input = sg.welch(x_unmixed_numpy, fs=fs, nperseg=nyquist, detrend='constant', axis=-1)
            spectrum_input = torch.tensor(spectrum_input[...,:-1], dtype=x_unmixed.dtype, device=x_unmixed.device)
        else:
            spectrum_input = 0
            for batch in dataloader:
                batch = batch.to(self.temporal_filter.weight.device)
                ...
                frequencies_input, spectrum_input_ = sg.welch(batch_unmixed_numpy, fs=fs, nperseg=nyquist, detrend='constant', axis=-1)
                spectrum_input_ = torch.tensor(spectrum_input_[...,:-1], dtype=batch_unmixed.dtype, device=batch_unmixed.device)
                spectrum_input += spectrum_input_ * batch_size
            spectrum_input /= len(dataset)

        temporal_filter = self.temporal_filter.weight.data
        frequencies_input = torch.tensor(frequencies_input[:-1], dtype=temporal_filter.dtype, device=temporal_filter.device)
        frequencies_filter, temporal_filter_amplitudes = self._spectrum(temporal_filter, fs, nfreq=nyquist)
        temporal_filter_amplitudes = einops.rearrange(temporal_filter_amplitudes, 'c 1 t -> c t')
        ...
        results = {
            ...
            'output_spectrum': torch.pow(temporal_filter_amplitudes, 2) * spectrum_input
        }
        return results